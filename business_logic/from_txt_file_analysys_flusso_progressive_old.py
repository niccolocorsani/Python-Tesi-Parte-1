...
def generate_lines(list_of_lines, img_name):
    ...
    for string_pattern in list_of_lines:
        ...
        try:
            ...
            if 'measu' not in string_pattern:
                ...
                #    elif 'p904b' or 'p954' or 's-901' in string_pattern:
                #       node_name = elements[0] #TODO capire perchè l'or logico funziona cosi male, stesso discorso per l'end logico
                else:  ## Nodo normale con più end
                    if len(elements) == 1:
                        raise Exception("Should be a start maybe!!")  # TODO rimetterlo una volta concluso
                    if '+' in elements[
            ...
            names_of_original_images_csv.append(node_name + ';DataProperty;isSchermata;' + img_name)
            ...
            if node_type == 'normal_one_end':
                if 'xv' in node_name:  ## sotto nodo del nodo normale: valvola
                    ...
                    to_csv.append(end + ';ObjectProperty;to;' + node_name)
    ...
    return nodes_csv, edges_csv, measures_csv, to_csv, names_of_original_images_csv
...
def aggiungi_etichetta_ai_doppioni(prefix, list):
    rottiObj = {'xv406': 4, 'awstart': 3, 'n2start2': 5, 'xv808': 4, 'hclstart': 3, 'cwexit1': 2, 'cwexit2': 2,
                'cwstart': 4, 'airstart': 3, 'degasamentoexit': 3, 'hclexit': 2, 'cl2start': 4, 'p602': 2, 'h2start': 3,
                'e-4505': 2, 'xv4235': 3, 'xv4236': 3, 'caminoexit': 2, 'degasamentoexit1': 2, 'awstart1': 2,
                'p601a': 2, 'cwexit': 2, 'acidoneexit': 2, 'p601b': 2, 'airstart2': 2, 'xv072c': 2, 'p600b': 2,
                'kohstart': 2, 'n2start1': 3, 'c905exit': 2, 'exit2': 2, 'abbacidoexit': 3, 's-4301start': 3,
                's-4302start': 3, 'p-4606start': 3, 'h2otorrestart1': 2, 'h2otorreexit1': 2, 'clorostart': 3,
                's4305exit': 2, 's4304exit': 2, 'degasamentoiposodioexit': 2, 'azotostart': 3, 'h2otorrestart': 2,
                'h2otorreexit': 2, 'h2ocaldastart': 2, 'r980': 2, 's-904e': 2, 's-904d': 2, 's-954b': 2, 's-954a': 2
                }

    for key in rottiObj.keys():
        for element in list:
            if key in element:
                list[list.index(element)] = element.replace(key, prefix + key)
...
if __name__ == '__main__':
    ...
    if os.path.exists(ROOT_DIR + '/' + 'all-nodes.txt'):
        os.remove(ROOT_DIR + '/' + 'all-nodes.txt')
    else:
        print("The file does not exist")
    ...
    path_name_original_images = os.listdir(ROOT_DIR + '/images_txt_incerto/')
    lines = []

    for path_name in path_name_original_images:

        lines = read_from_file_and_place_in_list(ROOT_DIR + '/images_txt_incerto/' + path_name)
        nodes_csv, edges_csv, measures_csv, to_csv, names_of_original_images_csv = generate_lines(lines, path_name)

        ####TODO aggiunto il 5 ottobre
        nodes_csv = aggiungi_etichetta_ai_doppioni(path_name, nodes_csv)
        edges_csv = aggiungi_etichetta_ai_doppioni(path_name, edges_csv)
        measures_csv = aggiungi_etichetta_ai_doppioni(path_name, measures_csv)
        to_csv = aggiungi_etichetta_ai_doppioni(path_name, to_csv)
        names_of_original_images_csv = aggiungi_etichetta_ai_doppioni(path_name, names_of_original_images_csv)
        ...
        f4 = open(ROOT_DIR + '/' + 'all-nodes.txt', 'a')
        ...
        try:
            ...
            f1 = open('/Users/nicc/Fe+Fe1+FE2+FeCl3 con to/' + path_name.replace('.txt', '.csv'), 'w')

            for element in nodes_csv:

                ################# Da rimuovere finita la tesi ##############################
                if 'CARICO_FERRICO_FERROSO_2' in path_name:
                    f.write('p904b;Class;Pump;')
                    f.write('\n')
                    f.write('p954;Class;Pump;')
                    f.write('\n')
                    f1.write('p904b;Class;Pump;')
                    f1.write('\n')
                    f1.write('p954;Class;Pump;')
                    f1.write('\n')

                if 'FERRICO_FERROSO_CLORO_FERRO' in path_name:
                    f.write('s-901;Class;Tank;')
                    f.write('\n')
                    f1.write('s-901;Class;Tank;')
                    f1.write('\n')
                ################# Da rimuovere finita la tesi ##############################
            ...
            for element in measures_csv:
                ...
                #### 9 luglio TODO da mettere nel caso (object Property To)
            ...
            ### 9 luglio
            for element in names_of_original_images_csv:
                f.write(element)
                f.write('\n')
                f1.write(element)
                f1.write('\n')
