...
def generate_lines(list_of_lines, img_name, counter_image):
    nodes_csv = []
    edges_csv = []
    measures_csv = []
    to_csv = []
    ...
    for string_pattern in list_of_lines:
        ...
        try:
            ...
            #       node_name = elements[0] #TODO capire perchè l'or logico funziona cosi male, stesso discorso per l'end logico
                    ...
                    if len(elements) == 1:
                        raise Exception("Should contain start")
                        node_name = elements[0].replace('node:','')
                        if(node_name == path_name): continue
                        nodes_csv.append(counter_image+'_'+node_name + ';Class;PhysicalObject;')
                        print(node_name+'------Lenght1')
                        print(path_name+'------Lenght1')
            ...
            names_of_original_images_csv.append(str(counter_image)+'_'+node_name + ';DataProperty;isSchermata;' + img_name)

            if node_type == 'normal_one_end':
                if 'xv' in node_name:  ## sotto nodo del nodo normale: valvola
                    nodes_csv.append(str(counter_image)+'_'+node_name + ';Class;Valve;')  # Nodo normale valvola
                    edges_csv.append('Flusso_' + str(flusso_num) + ';Class;Flow;')
                    edges_csv.append('Flusso_' + str(flusso_num) + ';ObjectProperty;startIn;' + str(counter_image)+'_'+end)
                    edges_csv.append('Flusso_' + str(flusso_num) + ';ObjectProperty;endIn;' +str(counter_image)+'_'+ node_name)
                    to_csv.append(end + ';ObjectProperty;to;' +str(counter_image)+'_'+ node_name)
                    flusso_num = flusso_num + 1
                else:
                    nodes_csv.append(str(counter_image)+'_'+node_name + ';Class;PhysicalObject;')  # Nodo normale non valvola
                    edges_csv.append('Flusso_' + str(flusso_num) + ';Class;Flow;')
                    edges_csv.append('Flusso_' + str(flusso_num) + ';ObjectProperty;startIn;' + str(counter_image)+'_'+end)
                    edges_csv.append('Flusso_' + str(flusso_num) + ';ObjectProperty;endIn;' +str(counter_image)+'_'+ node_name)
                    to_csv.append(end + ';ObjectProperty;to;' + str(counter_image)+'_'+node_name)
            ...
            if node_type == 'normal_multiple_end':
                ...
                for end in ends:
                    ...
                    edges_csv.append('Flusso_' + str(flusso_num) + ';ObjectProperty;startIn;' +str(counter_image)+'_'+ end)
                    edges_csv.append('Flusso_' + str(flusso_num) + ';ObjectProperty;endIn;' +str(counter_image)+'_'+ node_name)
                    to_csv.append(str(counter_image)+'_'+end + ';ObjectProperty;to;' +str(counter_image)+'_'+ node_name)
                    flusso_num = flusso_num + 1
            if node_type == 'measure_one':
                if '%' in type:
                    type = 'percentage'
                measures_csv.append(str(counter_image)+'_'+node_name + ';Class;Sensor;')
                measures_csv.append(type + ';Class;UnitOfMeasure;')
                measures_csv.append(str(counter_image)+'_'+node_name + ';ObjectProperty;canMeasureIn;' + type)
                measures_csv.append(str(counter_image)+'_'+node_name + ';ObjectProperty;isContainedIn;' + measure_of)
            ...
            if node_type == 'measure_multiple':
                ...
                for measure in measures:
                    measures_csv.append(str(counter_image)+'_'+node_name + ';ObjectProperty;isContainedIn;' + measure)
        except Exception as ex:
            if( '?' in string_pattern): continue
...
if __name__ == '__main__':
    ...
    path_name_original_images = os.listdir(ROOT_DIR + '/image_txt/')
    lines = []

    counter_image = 1
    for path_name in path_name_original_images:

        lines = read_from_file_and_place_in_list(ROOT_DIR + '/image_txt/' + path_name)
        nodes_csv, edges_csv, measures_csv, to_csv , names_of_original_images_csv = generate_lines(lines, path_name, counter_image)
